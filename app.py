...
# 1. 티커 리스트 (안정적인 데이터 로딩을 위해 확인된 티커 사용)
watch_list = {
    "KODEX 200": "069500.KS",
...
if data_dict:
    ...
    with col_set:
        ...
        with sc1:
            # key를 부여하여 상태를 명확히 관리
            target_stock = st.selectbox("종목 선택", list(data_dict.keys()), key="stock_selector")
        with sc2:
            p_map = {"1주": 5, "2주": 10, "1개월": 20, "3개월": 60, "6개월": 120, "1년": 252, "3년": 756, "5년": 1260, "10년": 2520}
            target_period = st.selectbox("기간 선택", list(p_map.keys()), index=3, key="period_selector")

    # 3. 차트 출력 (변수가 확실히 정의되었을 때만)
    if target_stock in data_dict:
    ...
    st.warning("데이터를 불러오는 중입니다. 잠시 후 새로고침 하거나 티커 정보를 확인해 주세요.")
