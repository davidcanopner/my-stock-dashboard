...
results = []
data_dict = {}
selected_stock = None # 에러 방지를 위해 미리 초기화
...
for name, ticker in watch_list.items():
    try:
        ...
        # 신호 판단
        rsi_status = f"🔴 과매수 ({rsi_val:.2f})" if rsi_val >= 70 else f"🔵 과매도 ({rsi_val:.2f})" if rsi_val <= 30 else f"⚪ 관망 ({rsi_val:.2f})"
